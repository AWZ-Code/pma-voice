...
RegisterNetEvent('ADDSERVER:ADMINTALK') --IL PUNE INCALL FOR ALL
AddEventHandler('ADDSERVER:ADMINTALK', function(mode)
	local _source = source
	local players = GetPlayers()
	for i = 1, #players, 1 do
		if players[i] ~= _source then
                print("MUTES")
			MumbleSetPlayerMuted(players[i], true)
        end        
	end
TriggerClientEvent("ADDCLIENT:ADMINTALK", -1, mode)
end)
